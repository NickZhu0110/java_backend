...
public class JobFileController {
    ...
    @PostMapping(
            value = "/corrected-mask",
            consumes = MediaType.MULTIPART_FORM_DATA_VALUE
    )
    public CorrectedMaskUploadResponse uploadCorrectedMask(
            @PathVariable Long jobId,
            @RequestPart("mask") MultipartFile maskFile,
            @RequestPart("metadata") MultipartFile metadataFile
    ) {
        // TODO: Before production, restrict corrected-mask artifact storage to a
        // configured root such as cac.storage.root and enforce authenticated users.
        log.info("Corrected mask upload requested: jobId={}, maskOriginalName={}, metadataOriginalName={}",
                jobId, maskFile.getOriginalFilename(), metadataFile.getOriginalFilename());
        return cacResultService.saveCorrectedMaskUpload(jobId, maskFile, metadataFile);
    }
