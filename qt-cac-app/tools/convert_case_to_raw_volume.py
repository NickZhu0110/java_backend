...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mode", choices=["both", "ct", "mask"], default="both")
...
def main() -> int:
    ...
    input_volume = Path(args.input_volume).expanduser().resolve() if args.input_volume else None
    ...
    if args.mode in {"both", "ct"}:
        if input_volume is None:
            raise RuntimeError("--input-volume is required for CT preprocessing")
        ...
        ct_metadata_path = output_dir / "input_volume" / "ct_volume_metadata.json"
        if not ct_metadata_path.exists():
            raise RuntimeError("ct_volume_metadata.json is required for mask-only preprocessing")
        ct_metadata = json.loads(ct_metadata_path.read_text(encoding="utf-8"))

    if args.mode in {"both", "mask"}:
        if mask_path is None:
            raise RuntimeError("--mask is required for mask preprocessing")
        ...
        json.dumps(
            {
                ...
                "mask_dimensions": [
                    ...
                    mask_metadata["depth"],
                ] if mask_metadata else None,
                "mask_matches_ct_dimensions": mask_metadata["matches_ct_dimensions"] if mask_metadata else None,
            },
            indent=2,
        )
    )
